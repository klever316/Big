...
function getCrimeDetalhes(cd_situacao, cod_foro) {
		var data = { det_situacao_key : cd_situacao, cod_foro_key : cod_foro }
		$.ajax({
...
function getReusDetalhes(cd_reus) {
        var data = { det_reus_key : cd_reus }
        $.ajax({
        url : "/crime_organizado/detalhereus",
        beforeSend: function(xhr) {xhr.setRequestHeader('X-CSRF-Token', $('meta[name="csrf-token"]').attr('content'))},
        type : "post",
        data : data,
        success: function(data, textStatus, jqXHR) {
            ...
            if (data.array.length > 0) {
                $("#corpo").empty();
                var html = "";
                html += '<table class="table datatable table-bordered dataTable no-footer" id="detalhe_reus" style="background-color: white">';
                html += '<thead>'
                html += '<tr>';
                html += '<th>Réus</th>';
                html += '<th>N° do Processo</th>';
                ...
                $(document).ready(function() {
                    var table_detalhe_reus = $('#detalhe_reus').DataTable({
                        dom: "<'row'<'col-sm-12'B>>" + "<'row'<'col-sm-6'l><'col-sm-6'f>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-5'i><'col-sm-7'p>>",
                        order: [[ 0, "asc" ]],
                        buttons: [
                        {
                            extend: 'copyHtml5', 
                            title: ('Detalhes dos Réus - ' + data.array[0].nmpessoa)
                        },
                        {
                            extend: 'csv', 
                            title: ('Detalhes dos Réus - ' + data.array[0].nmpessoa),
                            filename: ('Detalhes dos Réus - ' + data.array[0].nmpessoa) 
                        },
                        {
                            extend: 'pdfHtml5', 
                            title: ('Detalhes dos Réus - ' + data.array[0].nmpessoa),
                ...
                $("#corpo").append(html);
                $('#detalhe_foro').empty();
                $('#detalhe_foro').append('Lista de Réus');
                $("#myModal").modal("show");
            } else {
                document.getElementById('foroinvalido').innerHTML = "<div class='alert alert-danger'>    <strong> Foro Inválido!</strong>    </div>";
            }   
