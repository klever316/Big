class ProcessosJulgados
			...
			@@consulta_julgados_por_competencia.each_with_index do |row, i|
				if(a == row["pedi_num_ano"])
					if row["orju_dsc_unidade"].to_s.include? "VARA DE FAMILIA DA COMARCA DE FORTALEZA"
						...
						@varas_familia += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						if !row["orju_dsc_unidade"].to_s.include? "6ª VARA DO JURI"
							...
							@varas_juri += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						end
						...
						if !row["orju_dsc_unidade"].to_s.include? "19ª VARA CRIMINAL DA COMARCA DE FORTALEZA"
							...
							@varas_criminais += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						end
						...
						if !row["orju_dsc_unidade"].to_s.include? "VARA CÍVEL DA COMARCA DE FORTALEZA-INATIVA"
							...
							@varas_civeis += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						end
						...
						@varas_fazenda += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						@varas_infancia += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						@varas_execucoes_penais += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						@varas_execucoes_fiscais += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						@varas_falencia += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						@varas_registros_publicos += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						@varas_sucessoes += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
						...
						@varas_toxico += [{ name: "#{x}", y: row['julgados'], key: "#{row["orju_bsq_chave_unidade"]}"}]
				...
				# if @count_insercao == 1
				# 	@competencias[@count_insercao][:color] = "#BB0000"
				# end
				...
				@total_competencias = {total_familia: 0,
										...
										total_falencia: 0,
										total_registros_publicos: 0,
										total_sucessoes: 0, total_toxico: 0,
										total_auditoria_militar: 0,
										total_penas_alternativas: 0,
										total_transito: 0}
		end
	...
	def execute_sql(sql)
  	...
  	def self.get_percentual_julgados(vara, ano)
		conn = BigDB.connection
		@@consulta_percentual_julgados_por_competencia ||= conn.select_all "select pedi_num_ano,
       orju.orju_bsq_chave_unidade, orju.orju_dsc_unidade,
       ((sum(prtc_qtd_julgado_conhecimento)/sum(prtc_qtd_pendente_baixa_conh))*100) percentual_julgados
...
where orju.orju_bsq_chave_segmento in ('1G', 'JFP') and pedi_num_ano in (#{ano.join(',')})
  and prtc_qtd_pendente_baixa_conh > 0
group by pedi_num_ano,
         orju.orju_bsq_chave_unidade, orju.orju_dsc_unidade
order by 1, 3"
		@competencias = Array.new
		@total_competencias = Hash.new
		...
		@count_insercao = 0
			...
			ano.each do |a|
				@@consulta_percentual_julgados_por_competencia.each_with_index do |row, i|
					if(a == row["pedi_num_ano"])
						if row["orju_dsc_unidade"].to_s.include? "VARA DE FAMILIA DA COMARCA DE FORTALEZA"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_familia] += row["percentual_julgados"].to_f
							@count_competencias[:count_familia] += 1
							@varas_familia += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DO JURI DA COMARCA"
							if !row["orju_dsc_unidade"].to_s.include? "6ª VARA DO JURI"
								x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
								@total_competencias[:total_juri] += row["percentual_julgados"].to_f
								@count_competencias[:count_juri] += 1
								@varas_juri += [["#{x}",row['percentual_julgados'].to_f]]
							end	
						elsif row["orju_dsc_unidade"].to_s.include? "VARA CRIMINAL DA COMARCA DE FORTALEZA"
							if !row["orju_dsc_unidade"].to_s.include? "19ª VARA CRIMINAL DA COMARCA DE FORTALEZA"
								x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
								@total_competencias[:total_criminal] += row["percentual_julgados"].to_f
								@count_competencias[:count_criminal] += 1
								@varas_criminais += [["#{x}",row['percentual_julgados'].to_f]]
							end	
						elsif row["orju_dsc_unidade"].to_s.include? "VARA CÍVEL DA COMARCA DE FORTALEZA"
							if !row["orju_dsc_unidade"].to_s.include? "VARA CÍVEL DA COMARCA DE FORTALEZA-INATIVA"
								x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
								@total_competencias[:total_civel] += row["percentual_julgados"].to_f
								@count_competencias[:count_civel] += 1
								@varas_civeis += [["#{x}",row['percentual_julgados'].to_f]]
							end
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DA FAZENDA PÚBLICA DA COMARCA DE FORTALEZA"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_fazenda] += row["percentual_julgados"].to_f
							@count_competencias[:count_fazenda] += 1
							@varas_fazenda += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DA INFÂNCIA E JUVENTUDE DA COMARCA DE FORTALEZA"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_infancia] += row["percentual_julgados"].to_f
							@count_competencias[:count_infancia] += 1
							@varas_infancia += [["#{x}",row['percentual_julgados'].to_f]]
						 elsif row["orju_dsc_unidade"].to_s.include? "VARA DE EXECUÇÃO PENAL DA COMARCA DE FORTALEZA"
						 	x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
						 	@total_competencias[:total_execucoes_penais] += row["percentual_julgados"].to_f
						 	@count_competencias[:count_execucoes_penais] += 1
						 	@varas_execucoes_penais += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DE EXECUÇÕES FISCAIS E DE CRIMES CONTRA A ORDEM TRIBUTÁRIA DA COMARCA DE FORTALEZA"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_execucoes_fiscais] += row["percentual_julgados"].to_f
							@count_competencias[:count_execucoes_fiscais] += 1
							@varas_execucoes_fiscais += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DE RECUPERAÇÃO DE EMPRESAS E FALÊNCIAS"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_falencia] += row["percentual_julgados"].to_f
							@count_competencias[:count_falencia] += 1
							@varas_falencia += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DE REGISTROS PÚBLICOS DA COMARCA DE FORTALEZA"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_registros_publicos] += row["percentual_julgados"].to_f
							@count_competencias[:count_registros_publicos] += 1
							@varas_registros_publicos += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DE SUCESSÕES DA COMARCA DE FORTALEZA"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_sucessoes] += row["percentual_julgados"].to_f
							@count_competencias[:count_sucessoes] += 1
							@varas_sucessoes += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DELITOS/TRAFICO SUBST. ENTORPECENTES COMARCA DE FORTALEZA"
							x = row["orju_dsc_unidade"].gsub(" DA COMARCA DE FORTALEZA", "")
							@total_competencias[:total_toxico] += row["percentual_julgados"].to_f
							@count_competencias[:count_toxico] += 1
							@varas_toxico += [["#{x}",row['percentual_julgados'].to_f]]
						elsif row["orju_dsc_unidade"].to_s.include? "VARA DA AUDITORIA MILITAR DA COMARCA DE FORTALEZA"
							@total_competencias[:total_auditoria_militar] += row["percentual_julgados"].to_f
							@count_competencias[:count_auditoria_militar] += 1
						 elsif row["orju_dsc_unidade"].to_s.include? "VARA DE EXECUÇÕES DE PENAS ALTERNATIVAS DE FORTALEZA"
						 	@total_competencias[:total_penas_alternativas] += row["percentual_julgados"].to_f
						 	@count_competencias[:count_penas_alternativas] += 1
						elsif row["orju_dsc_unidade"].to_s.include? "VARA ÚNICA DE TRÂNSITO DA COMARCA DE FORTALEZA"
							@total_competencias[:total_transito] += row["percentual_julgados"].to_f
	...
	def self.get_percentual_julgados_forum(ano)
		conn = BigDB.connection
		@@consulta_percentual_julgados_forum_ano ||= conn.select_all "select pedi_num_ano,
       ((sum(prtc_qtd_julgado_conhecimento)/sum(prtc_qtd_pendente_baixa_conh))*100) percentual_julgados_forum_ano
from dwfcb.pa_prtc_processo_taxa_cong prtc
     join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave
     join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia
where orju.orju_bsq_chave_segmento in ('1G', 'JFP')
  and prtc_qtd_pendente_baixa_conh > 0 AND pedi_num_ano in (#{ano.join(',')})
group by pedi_num_ano
order by 1"
		@@consulta_percentual_julgados_forum_mes ||= conn.select_all "select pdrf.pedi_seq_chave, pedi_num_ano, pdrf.pedi_dsc_mes,
       ((sum(prtc_qtd_julgado_conhecimento)/sum(prtc_qtd_pendente_baixa_conh))*100) percentual_julgados_forum_mes
from dwfcb.pa_prtc_processo_taxa_cong prtc
     join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave
     join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia
where orju.orju_bsq_chave_segmento in ('1G', 'JFP')
  and prtc_qtd_pendente_baixa_conh > 0 AND pedi_num_ano in (#{ano.join(',')})
group by  pdrf.pedi_seq_chave, pdrf.pedi_dsc_mes, pedi_num_ano
order by 1"
		@ano = Array.new
		@mes = Array.new
		@dados = Array.new
		count = 0
		@@consulta_percentual_julgados_forum_ano.each do |row|
			if(count == 0)
				@ano << {name: "#{row['pedi_num_ano']}", data: [{name: "Ano", y: row['percentual_julgados_forum_ano'].to_f, drilldown: "#{row['pedi_num_ano']}"}], color: "#8CD19E"}
			elsif(count == 1)
				@ano << {name: "#{row['pedi_num_ano']}", data: [{name: "Ano", y: row['percentual_julgados_forum_ano'].to_f, drilldown: "#{row['pedi_num_ano']}"}], color: "#8dd7e0"}
			elsif(count == 2)
				@ano << {name: "#{row['pedi_num_ano']}", data: [{name: "Ano", y: row['percentual_julgados_forum_ano'].to_f, drilldown: "#{row['pedi_num_ano']}"}], color: "#db6a29"}
			end
			count += 1
		end
		ano.each do |a|	
			@@consulta_percentual_julgados_forum_mes.each do |row|
				if(a==row['pedi_num_ano'])
					@dados << ["#{row['pedi_dsc_mes'].gsub(" ","")}", row['percentual_julgados_forum_mes'].to_f]
				end
			end
	...
	def self.get_total_julgados
		...
		@@total_pendentes = nil
		return @total_julgados
	end

	def self.get_lista_processos_julgados(chave_unidade, ano)
		conn = BigDB.connection
		@lista_processos = conn.select_all("select pedi_num_ano, ORJU_BSQ_CHAVE_UNIDADE, orju_dsc_unidade, PROC_DSC_PROCESSO_FORMATADO, SIPR_DSC_SITUACAO_PROCESSO, PROC_DAT_PROTOCOLO from dwfcb.pf_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia join dwfcb.pd_proc_processo proc on proc.proc_seq_chave =  prtc.proc_seq_chave join dwfcb.pd_sipr_situacao_processo sit on sit.sipr_seq_chave  = prtc.sipr_seq_chave	 where orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{ano}) and orju_bsq_chave_unidade like '#{chave_unidade}' and PRTC_QTD_JULGADO_CONHECIMENTO = 1")
