class ProcessosPendentes
	...
	def self.get_processos_pendentes_por_competencia(vara,ano)
		...
		if(Date.today.strftime("%m").to_i == 1)
			if Date.today.strftime("%d").to_i <= 15
				@@consulta_pendentes_por_competencia ||= conn.select_all "select pedi_num_ano, orju_dsc_unidade_pai, orju_bsq_chave_unidade, ORJU_DSC_UNIDADE, sum(prtc_qtd_pendente_baixa_conh) as pendentes from dwfcb.pa_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia where orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{ano.join(',')}) AND pedi_num_mes IN (11) group by pedi_num_ano, orju_dsc_unidade_pai, orju_bsq_chave_unidade, ORJU_DSC_UNIDADE order by 1, 2"
			else
				@@consulta_pendentes_por_competencia ||= conn.select_all "select pedi_num_ano, orju_dsc_unidade_pai, orju_bsq_chave_unidade, ORJU_DSC_UNIDADE, sum(prtc_qtd_pendente_baixa_conh) as pendentes from dwfcb.pa_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia where orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{ano.join(',')}) AND pedi_num_mes IN (12) group by pedi_num_ano, orju_dsc_unidade_pai, orju_bsq_chave_unidade, ORJU_DSC_UNIDADE order by 1, 2"
			end
	...
	def self.get_processos_pendentes(ano)
		...
		@lista_processos_pendentes = Array.new
		...
		ano.each do |a|
			...
			if(@count_insercao == 0)
				@lista_processos_pendentes << {name: "#{a}", data: @processos_pendentes_ano, color: '#8CD19E'}
			elsif(@count_insercao == 1)
				@lista_processos_pendentes << {name: "#{a}", data: @processos_pendentes_ano, color: '#8dd7e0'}
			elsif(@count_insercao == 2)
				@lista_processos_pendentes << {name: "#{a}", data: @processos_pendentes_ano, color: '#db6a29'}
			end
	...
	def self.get_total_pendentes
		...
		if Date.today.strftime("%m").to_i == 1
			...
			if @@total_pendentes[0]['prtc_qtd_pendente_baixa_conh'].nil?
			...
			puts @@total_pendentes[0]['prtc_qtd_pendente_baixa_conh']
			if @@total_pendentes[0]['prtc_qtd_pendente_baixa_conh'].nil?
				@@total_pendentes = conn.select_all("select sum(prtc_qtd_pendente_baixa_conh) qtd_pendente_baixa_conh FROM dwfcb.pa_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju ON orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia WHERE orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{Date.current.year-1}) AND pedi_num_mes IN (12)")
			end
		else
		...
		# Método antigo pegava até o dia 15 pra atualizar os dados
		# if Date.today.strftime("%d").to_i <= 15
	...
	def self.get_lista_processos_pendentes(chave_unidade, ano)
		...
		if ano == Date.today.year
			if Date.today.strftime("%m").to_i == 1
				@consulta_lista_processos = conn.select_all  "select pedi_num_ano, ORJU_BSQ_CHAVE_UNIDADE, orju_dsc_unidade, PROC_DSC_PROCESSO_FORMATADO, SIPR_DSC_SITUACAO_PROCESSO, PROC_DAT_PROTOCOLO from dwfcb.pf_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia  join dwfcb.pd_proc_processo proc on proc.proc_seq_chave =  prtc.proc_seq_chave join dwfcb.pd_sipr_situacao_processo sit on sit.sipr_seq_chave  = prtc.sipr_seq_chave  where orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{ano-1}) and pedi_num_mes IN (12) and orju_bsq_chave_unidade like '#{chave_unidade}' and PRTC_QTD_PENDENTE_BAIXA_CONH = 1"
				if @consulta_lista_processos[0].nil?
					@consulta_lista_processos = conn.select_all  "select pedi_num_ano, ORJU_BSQ_CHAVE_UNIDADE, orju_dsc_unidade, PROC_DSC_PROCESSO_FORMATADO, SIPR_DSC_SITUACAO_PROCESSO, PROC_DAT_PROTOCOLO from dwfcb.pf_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia  join dwfcb.pd_proc_processo proc on proc.proc_seq_chave =  prtc.proc_seq_chave join dwfcb.pd_sipr_situacao_processo sit on sit.sipr_seq_chave  = prtc.sipr_seq_chave  where orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{ano-1}) and pedi_num_mes IN (11) and orju_bsq_chave_unidade like '#{chave_unidade}' and PRTC_QTD_PENDENTE_BAIXA_CONH = 1"
				end
			elsif Date.today.strftime("%m").to_i == 2
