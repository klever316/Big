class ProcessosPendentes
	...
	def self.get_processos_pendentes_por_competencia(vara,ano)
		...
		@@consulta_pendentes_por_competencia ||= conn.select_all "select pedi_num_ano, orju_dsc_unidade_pai, ORJU_DSC_UNIDADE, sum(prtc_qtd_pendente_baixa_conh) as pendentes from dwfcb.pa_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia where orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{ano.join(',')}) AND pedi_num_mes IN (8) group by pedi_num_ano, orju_dsc_unidade_pai, ORJU_DSC_UNIDADE order by 1, 2"
				...
				@@consulta_pendentes_por_competencia.each_with_index do |row, i|
					if(a == row["pedi_num_ano"])
						if row["orju_dsc_unidade"].to_s.include? "VARA DE FAMILIA DA COMARCA DE FORTALEZA"
							...
							@varas_familia += [["#{x}".gsub!("?","ª"),row['pendentes']]]
							...
							if !row["orju_dsc_unidade"].to_s.include? "6? VARA DO JURI"
								...
								@varas_juri += [["#{x}".gsub!("?","ª"),row['pendentes']]]
							end
							...
							if !row["orju_dsc_unidade"].to_s.include? "19? VARA CRIMINAL DA COMARCA DE FORTALEZA"
								...
								@varas_criminais += [["#{x}".gsub!("?","ª"),row['pendentes']]]
							end
							...
							if !row["orju_dsc_unidade"].to_s.include? "VARA CIVEL DA COMARCA DE FORTALEZA-INATIVA"
								...
								@varas_civeis += [["#{x}".gsub!("?","ª"),row['pendentes']]]
							end
							...
							@varas_fazenda += [["#{x}".gsub!("?","ª"),row['pendentes']]]
							...
							@varas_infancia += [["#{x}".gsub!("?","ª"),row['pendentes']]]
							...
							@varas_execucoes_penais += [["#{x}".sub!("?","Ã"),row['pendentes']]]
							...
							@varas_execucoes_fiscais += [["#{x}".sub!("?","Õ"),row['pendentes']]]
							...
							@varas_falencia += [["#{x}".sub!("?","Ã"),row['pendentes']]]
							...
							@varas_registros_publicos += [["#{x}".gsub!("?","ª"),row['pendentes']]]
							...
							@varas_sucessoes += [["#{x}".sub!("?","Õ"),row['pendentes']]]
							...
							@varas_toxico += [["#{x}".gsub!("?","ª"),row['pendentes']]]
