class ProcessosPendentes
	...
	def self.get_processos_pendentes_por_competencia(vara,ano)
		...
		@@consulta_pendentes_por_competencia ||= conn.select_all "select pedi_num_ano, orju_dsc_unidade_pai, ORJU_DSC_UNIDADE, sum(prtc_qtd_pendente_baixa_conh) as pendentes from dwfcb.pa_prtc_processo_taxa_cong prtc join dwfcb.pd_orju_orgao_julgador orju on orju.orju_seq_chave = prtc.orju_seq_chave join dwfcb.cd_pedi_periodo_diario pdrf on pdrf.pedi_seq_chave = prtc.pedi_seq_chave_referencia where orju.orju_bsq_chave_segmento in ('1G', 'JFP') AND pedi_num_ano IN (#{ano.join(',')}) AND pedi_num_mes IN (To_Char(SYSDATE,'MM')-1) group by pedi_num_ano, orju_dsc_unidade_pai, ORJU_DSC_UNIDADE order by 1, 2"
			...
			@@consulta_pendentes_por_competencia.each_with_index do |row, i|
				if(a == row["pedi_num_ano"])
					if row["orju_dsc_unidade"].to_s.include? "VARA DE FAMILIA DA COMARCA DE FORTALEZA"
						...
						@varas_familia += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						if !row["orju_dsc_unidade"].to_s.include? "6ª VARA DO JURI"
							...
							@varas_juri += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						end
						...
						if !row["orju_dsc_unidade"].to_s.include? "19ª VARA CRIMINAL DA COMARCA DE FORTALEZA"
							...
							@varas_criminais += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						end
						...
						if !row["orju_dsc_unidade"].to_s.include? "VARA CÍVEL DA COMARCA DE FORTALEZA-INATIVA"
							...
							@varas_civeis += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						end
						...
						@varas_fazenda += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						@varas_infancia += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						@varas_execucoes_penais += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						@varas_execucoes_fiscais += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						@varas_falencia += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						@varas_registros_publicos += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						@varas_sucessoes += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
						...
						@varas_toxico += [{ name: "#{x}", y: row['pendentes'], key: "#{row["orju_dsc_unidade"]}"}]
