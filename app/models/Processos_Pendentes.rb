					...
					if(a == row["pedi_num_ano"])
						if row["orju_dsc_unidade"].to_s.include? "VARA DE FAMILIA DA COMARCA DE FORTALEZA"
							...
							if !row["orju_dsc_unidade"].to_s.include? "19ª VARA CRIMINAL DA COMARCA DE FORTALEZA"
								...
								@varas_criminais += [["#{x}",row['pendentes']]]
							end
							...
							@varas_infancia += [["#{x}",row['pendentes']]]
							...
							@varas_execucoes_penais += [["#{x}",row['pendentes']]]
							...
							@varas_execucoes_fiscais += [["#{x}",row['pendentes']]]
							...
							@varas_falencia += [["#{x}",row['pendentes']]]
							...
							@varas_registros_publicos += [["#{x}",row['pendentes']]]
							...
							@varas_sucessoes += [["#{x}",row['pendentes']]]
